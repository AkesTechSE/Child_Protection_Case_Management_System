...
class ReportController extends Controller
{
    ...
    public function casesReport(Request $request)
    {
        ...
        $cases = $query->with(['victims', 'perpetrators', 'assignedTo:id,name,email', 'incidents'])
    ...
    public function incidentReport(Request $request)
    {
        ...
        $incidents = Incident::whereDate('incident_datetime', '>=', $start)
            ...
            ->with(['case:id,case_number,case_title,status'])
            ->get();
        ...
        $summary = [
            ...
            'time_of_day' => [
                'morning' => $incidents->filter(fn($i) => Carbon::parse($i->incident_datetime)->hour >= 6 && Carbon::parse($i->incident_datetime)->hour < 12)->count(),
                'afternoon' => $incidents->filter(fn($i) => Carbon::parse($i->incident_datetime)->hour >= 12 && Carbon::parse($i->incident_datetime)->hour < 18)->count(),
                'evening' => $incidents->filter(fn($i) => Carbon::parse($i->incident_datetime)->hour >= 18 && Carbon::parse($i->incident_datetime)->hour < 22)->count(),
                'night' => $incidents->filter(fn($i) => Carbon::parse($i->incident_datetime)->hour >= 22 || Carbon::parse($i->incident_datetime)->hour < 6)->count(),
            ],
    ...
    public function comprehensiveReport(Request $request)
    {
        ...
        $cases = AbuseCase::whereDate('created_at', '>=', $start)
            ->whereDate('created_at', '<=', $end)
            ->with(['victims', 'perpetrators', 'incidents', 'assignedTo'])
            ->get();
        ...
        $summary = [
            ...
            'incidents' => [
                ...
                'by_type' => $incidents->groupBy('abuse_type')->map->count(),
                'by_location' => $incidents->groupBy('location_type')->map->count(),
            ],
            'timeline' => [
